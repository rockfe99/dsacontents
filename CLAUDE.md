...
- **호출 실패 시 화면 처리**: API 호출 오류, 타임아웃, 사용량/쿼터 초과 등 원인이
  무엇이든 `callAI_()`를 호출하는 쪽(요약·시험문제 생성·이해도 보고서 등)은 그
  오류를 잡아(try/catch) **"현재 AI 기능을 사용할 수 없습니다."** 같은 고정 안내
  문구만 사용자에게 보여준다. 원본 에러 메시지·스택트레이스를 그대로 노출하지
  않는다(내부 원인은 `Logger.log()`로만 남긴다). 이 기능이 실패해도 배포·목차
