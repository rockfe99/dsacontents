...
create table virtual_question_personas (
  ...
  exam_level    text check (exam_level in ('beginner','intermediate','advanced')),
                                          -- 시험문제 생성 시 이 페르소나의 가상질문을 참고자료로
                                          -- 쓸 시험 난이도. 이 페르소나가 실제로 하는 질문의 성격이
                                          -- 어느 시험 수준과 어울리는지로 정한다(이해도 서열과는 별개
                                          -- 축 - 예: 실무경험자는 이해도는 높지 않아도 질문 자체는
                                          -- 종합응용형이라 고급에 배정)
...
insert into virtual_question_personas (persona_id, name, description, prompt, display_order, exam_level) values
('beginner', '초심자',
